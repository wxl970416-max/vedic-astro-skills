...
5. **写入防卡死**：
   - 每次使用Codex文件编辑工具写入控制在**250行以内**
   - 如果内容超250行，先写前半部分，然后用追加模式写后半部分
   - 行星审计：每2-3颗星写一次，不要强行一口气写入
   - 如果文件写入失败，立刻拆分为更小的块重试，不要反复重试同样的大块
...
```
⚠️⚠️⚠️ 阶段2开始前，必须用Codex文件读取工具重读以下文件：
...
**开始前先回调数据**：用Codex文件读取工具重读p2a/p2b/p2c/p2d_planets.md中的行星审计表格，确保宫位分析引用的是精确数据而非对话记忆。
...
**开始前先回调数据**：用Codex文件读取工具重读structured_data.md和已完成的p2a~p2d、p3a/p3b、p4a/p4b文件的关键结论，不要凭记忆。
