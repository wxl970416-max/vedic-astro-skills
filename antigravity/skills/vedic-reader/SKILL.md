...
1. 从用户提供的星盘材料中提取数据，或读取 vedic-calculator 已生成的 structured_data.md
...
### PDF/文本模式（传统模式，借力calc）

```
阶段1（数据提取）: Steps 0-3.5 + 借力calc补算 → WRITE 1 → 输出进度
...
#### 1.5 分盘数据提取（借力calc，不再需要截图）

**⚠️ 重大变更：分盘数据现在由 vedic-calculator engine 直接计算，不再需要用户截图！**

```
提取范围：D9 + D10 + D4 + D5（固定四个）

获取方式（按优先级）：

  方式A — calc engine 直接计算（推荐，100%准确）：
    → 从PDF/文本中提取出生日期、时间、地点
    → 调用 vedic-calculator engine 的 calculate_full_chart()
    → 获得 D9/D10/D4/D5 全部行星位置（与JHora 100%一致）
    → 标注"calc engine直接计算"
    → 同时获得：SAV/BAV、宫主表、尊贵度、相位、过运等全部数据
...
PDF/文本模式（借力calc）：calc engine 在 Step 1.5 调用时已同时计算过运，
